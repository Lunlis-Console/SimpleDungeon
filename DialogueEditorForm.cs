...
{
    /// <summary>
    /// Полный готовый файл DialogueEditorForm.cs — замените существующий файл на этот.
    /// Требует Newtonsoft.Json и файлы моделей DialogueDocument/DialogueNode/Response/DialogueAction
    /// из SimpleDungeon.Engine.Dialogue.
    /// 
    /// Особенности:
    /// - Безопасная загрузка game_data.json (поддержка контейнеров с "dialogs" или "dialogues").
    /// - Save (сохранить в текущий файл) всегда вставляет/заменяет диалог внутри массива Dialogues (создаёт поле, если его нет).
    /// - Save As — сохранить в отдельный файл.
    /// - Публичные методы LoadFromPath, SaveToPath и LoadFileProgrammatically — удобны для интеграции в JsonEditor.
    /// - Защита от непреднамеренной перезаписи файлов.
    /// </summary>
    public class DialogueEditorForm : Form
    {
        ...
        public DialogueEditorForm()
        {
            ...
            _docIdBox = new TextBox { Left = 70, Top = 628, Width = 250 };
            _docIdBox.Leave += (s, e) => { if (_doc != null) _doc.Id = _docIdBox.Text?.Trim(); };
            Controls.Add(_docIdBox);

            _filePathBox = new TextBox { Left = 330, Top = 628, Width = 450, ReadOnly = true };
            Controls.Add(_filePathBox);

            _saveCurrentBtn = new Button { Left = 790, Top = 624, Width = 80, Text = "Save" };
            _saveCurrentBtn.Click += (s, e) => SaveToCurrentFile();
            Controls.Add(_saveCurrentBtn);

            _statusLabel = new Label { Left = 880, Top = 628, Width = 90, Text = "" };
        ...
        private void RefreshNodesList()
        {
            ...
            _statusLabel.Text = _loadedJObject != null ? "container" : "";
        ...
        // -------------------------
        /// <summary>
        /// Программно загрузить файл (без OpenFileDialog).
        /// </summary>
        ...
        /// <summary>
        /// Открыть редактор для конкретного диалога в контейнерном game_data.json:
        /// - загружает JObject файла (чтобы при сохранении сохранить остальные поля),
        /// - устанавливает текущий документ редактора в переданный dialog (dialog.Id должен быть задан).
        /// После ShowDialog() пользователь может нажать Save чтобы применить изменения в том же файле game_data.json.
        /// </summary>
        public void EditDialogFromGameData(string gameDataPath, SimpleDungeon.Engine.Dialogue.DialogueDocument dialog)
        {
            ...
            // Попробуем прочитать контейнерный файл
            try
            {
                ...
                {
                    // если файл не объект — будем перезаписывать при сохранении
                    _loadedJObject = null;
                    _currentFilePath = gameDataPath;
                }
            }
            ...
            // установить документ в редактор
            _doc = dialog;
        ...
        /// <summary>
        /// Загружает файл по пути (без UI OpenFileDialog). Корректно заполняет _loadedJObject если файл — контейнер.
        /// </summary>
        public void LoadFromPath(string path)
        {
            ...
            if (token.Type == JTokenType.Object)
            {
                ...
                bool hasDialogsArray = jobj["dialogs"] != null && jobj["dialogs"].Type == JTokenType.Array;
                bool hasDialoguesArray = jobj["dialogues"] != null && jobj["dialogues"].Type == JTokenType.Array;

                if (looksLikePlainDialogue && !hasDialogsArray && !hasDialoguesArray && jobj.Properties().Count() <= 4)
                {
                ...
                {
                    var array = (JArray)(hasDialogsArray ? jobj["dialogs"] : jobj["dialogues"]);
                    if (array.Count == 0)
                    {
        ...
        public void SaveToPath(string path)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    // Если файл существует, но не валидный JSON — сообщаем и прекращаем, чтобы не потереть данные
            ...
            if (parsedOriginal && original != null)
            {
                ...
                if (arr != null)
                {
                    ...
                    {
                        // на платформах, где File.Replace не поддерживается, делаем fallback
