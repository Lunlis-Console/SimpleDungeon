...
namespace Engine.UI
{
    public class RoomInteractionScreen : BaseScreen
    {
        ...
        private List<WorldEntity> GetInteractableEntities()
        {
            ...
            // Сундуки
            foreach (var chest in _room.ChestsHere)
            {
        ...
        private void RenderRoomEntranceInfo(RoomEntrance entrance, int x, ref int y)
        {
            _renderer.Write(x, y, $"Тип: {entrance.EntranceType}");
            y++;
            _renderer.Write(x, y, $"Описание: {entrance.Description}");
            y += 2;
            ...
            if (entrance.IsLocked)
            {
                ...
                y++;
                if (!string.IsNullOrEmpty(entrance.LockDescription))
                {
                    _renderer.Write(x, y, entrance.LockDescription);
                    y++;
                }
                y++;
            }
            ...
            if (entrance.IsLocked)
            {
                _renderer.Write(x, y, "• Попытаться открыть");
                y++;
            }
            else
            {
                _renderer.Write(x, y, "• Войти");
                y++;
            }
            _renderer.Write(x, y, "• Осмотреть");
        }
        ...
        private void InteractWithMonster(Monster monster)
        {
            ...
            if (_pendingScreenToPush != null)
            {
                ...
                _pendingScreenToPush = null;
                ScreenManager.RequestFullRedraw();
                DebugConsole.Log("[взаимодействие] отправлен экран осмотра");
            }
        }
        ...
        private void InteractWithRoomEntrance(RoomEntrance entrance)
        {
            var actions = entrance.GetAvailableActions(_player);
            actions.Add("Назад");

            ShowActionMenu($"Взаимодействие с {entrance.GetDisplayName()}", actions, (selectedAction) =>
            {
                if (selectedAction != "Назад")
                {
                    entrance.ExecuteAction(_player, selectedAction);
                    ScreenManager.RequestPartialRedraw();
                }
            });
        }
